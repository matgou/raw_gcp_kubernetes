...
# Install binaries
gsutil ls gs://${cluster_binary_bucket}/bin/** | while read fic; do gsutil cp $fic /tmp/; cd /usr/local/bin/; tar xvf /tmp/$(basename $fic); done;
gsutil ls gs://${cluster_binary_bucket}/config/** | while read fic; do gsutil cp $fic $(echo $fic | sed s@gs://${cluster_binary_bucket}/config@@); done; 
